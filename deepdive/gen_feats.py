...
def get_seq_definition_features(sentence, span1, span2, length_bin_size=5):
    ...
    for window_feat in _get_window_features1(sentence, span1,span2,window = 2):
        yield window_feat
...
def _get_relation_token_feature(sentence,span1,span2):
    try:
        for i in {1, 2}:
            token = str(sentence[span1.begin_word_id -i + span1.length].word)
            pos = str(sentence[span2.begin_word_id - i - 1].pos)
            yield "LEFT_TOKEN_" + str(i) + '_' + toLowerCase(token)
            #yield "LEFT_POS_" + str(i) + '_' + pos
    except IndexError:
        pass
    try:
        for i in {1, 2}:
...
def _get_window_features1 (sentence, span1, span2, window=3, combinations=True, isolated=True):
    ...
    try:
        for i in range(1, window + 1):
            pos = str(sentence[span2.begin_word_id -1 + i].pos)
            right_pos.append(pos)
    except IndexError:
        pass
    if isolated:
...
def get_forbidden_word(sentence, span,span1 ) :
    forbidden_word = ['nếu',"đối_với","trường_hợp",'phải','đó','không','được','đã','đồng_thời','cần', 'chỉ','cụ_thể','coi','đây','ai']
    sent = ""
